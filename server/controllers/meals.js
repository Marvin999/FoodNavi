...
import * as recipeService from '../services/mealService.js'
...
export const handleGetRecipe = async (request, response) => {
    ...
    response.status(404).send({failed: "Meal not found"})
...
export const handleCreateRecipe = async (request,response) => {
    ...
    if(request.body.name == "") {
        response.status(400).send({required: "Meals Name required!"})
        return
    }
    ...
    response.status(422).send({failed: "Failed to save Meal"});
}
...
export const handleUpdateRecipe = async (request, response) => {
    ...
    if (returnCard) {
        response.status(200).send( {success: "updated Meal"})
        return
    }

    response.status(422).send({failed: "Failed to Update Meal"})
